...
class PlacedTower {
  ...
  double get attackSpeed =>
      currentUpgrade?.attackSpeed ?? towerData.attackSpeed;
  double? get slowAmount => currentUpgrade?.slowAmount ?? towerData.slowAmount;
  int? get incomePerSecond =>
      currentUpgrade?.incomePerSecond ?? towerData.incomePerSecond;
...
class TDGameEngine {
  ...
  void _updateTowers(double deltaTime) {
    for (final tower in placedTowers) {
      ...
      if (tower.currentTarget == null ||
          ...
          tower.currentTarget!.reachedEnd) {
  ...
  void _spawnNextEnemy(Wave wave) {
    ...
    for (final waveEnemy in wave.enemies) {
      if (enemiesSpawnedInWave >= currentCount &&
          enemiesSpawnedInWave < currentCount + waveEnemy.count) {
  ...
  void _checkGameState() {
    if (ipAssetHealth <= 0) {
      isGameOver = true;
      isVictory = false;
    } else if (currentWaveIndex >= levelData.waves.length &&
  ...
  void sellTower(PlacedTower tower) {
    ...
    for (int i = 2; i <= tower.level; i++) {
      totalCost += tower.towerData.upgrades
          .firstWhere((u) => u.level == i)
          .cost;
    }
