...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<int> _getStudentCount() async {
    ...
    try {
      final studentIds =
          (_classroomInfo!['studentIds'] as List?)?.cast<String>() ?? [];
      final teacherId = _classroomInfo!['teacherId'] as String?;
      final teacherIds =
          (_classroomInfo!['teacherIds'] as List?)?.cast<String>() ?? [];
    ...
    try {
      ...
      try {
        ...
        for (final doc in sortedDocs.take(3)) {
          ...
          final lastAccessedAt = (data['lastAccessedAt'] as Timestamp?)
              ?.toDate();
      ...
      try {
        ...
        for (final doc in sortedDocs.take(3)) {
          ...
          final lastScore =
              data['lastScore'] as int? ?? data['highScore'] as int? ?? 0;

          if (lastPlayedAt != null && gameId != null) {
            // Format game name from ID
            final gameName = gameId
                .split('_')
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Realms Completed: ${_user?.progressSummary.values.where((r) => r.completed).length ?? 0}',
            ),
          ],
        ),
        actions: [
...
class ColoredCard extends StatelessWidget {
  ...
  const ColoredCard({super.key, required this.color, required this.child});
