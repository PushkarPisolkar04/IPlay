    ...
    try {
      ...
      if (_hasSchoolCode && _schoolCodeController.text.trim().isNotEmpty) {
        ...
        try {
          ...
          if (schoolData['status'] != null &&
              schoolData['status'] != 'active') {
            if (!mounted) return;
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'This school is not active. Please contact the administrator.',
                ),
                backgroundColor: Colors.red,
              ),
            );
      ...
      if (isPrincipal) {
        ...
        await FirebaseFirestore.instance
            .collection('users')
            .doc(user.uid)
            .update({'principalOfSchool': schoolId});
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: RadioListTile<bool>(
                          ...
                          onChanged: (value) =>
                              setState(() => _hasSchoolCode = value ?? false),
                          title: Text(
                      ...
                      Container(
                        decoration: BoxDecoration(
                          color: AppDesignSystem.primaryPink.withValues(
                            alpha: 0.1,
                          ),
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                        ...
                        child: RadioListTile<bool>(
                          ...
                          groupValue: _hasSchoolCode,
                          onChanged: (value) =>
                              setState(() => _hasSchoolCode = value ?? true),
                          title: Row(
                            children: [
                              Text(
                                'Create New School',
